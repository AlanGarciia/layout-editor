...
import FileCard, { formatSize } from "./FileCard.jsx";
import PresetSelector from "./PresetSelector.jsx";
import { optimizeImage } from "./api.js";

/*
 * Compressor (Fase 2)
 * -------------------
 * Sube archivos de diseno y los optimiza:
 *   - PNG/JPG/WebP -> al backend (Pillow + pngquant) segun el preset.
 *   - SVG          -> optimizado en el navegador con SVGO.
 *
 * Cada archivo va a una cola con su propio estado. "Optimizar todo" procesa
 * los pendientes; cada resultado se puede descargar por separado o todos juntos.
 */
  ...
  plugins: [
    {
      ...
      params: {
        overrides: {
          // no quitar el viewBox (necesario para escalar bien)
          removeViewBox: false,
        },
      },
    },
...
export default function Compressor() {
  ...
  // Anade archivos nuevos a la cola
  ...
  // Procesa un unico item segun su tipo
  const processItem = async (item) => {
    const isSvg = item.file.name.toLowerCase().endsWith(".svg");

    if (isSvg) {
      // --- SVG: optimizar en el navegador con SVGO ---
  ...
  const runAll = async () => {
    setProcessing(true);
    // capturamos los items pendientes (con su File) antes de empezar
    ...
    for (const target of pending) {
      ...
      try {
        ...
        setItems((prev) =>
          prev.map((i) =>
            i.id === target.id ? { ...i, status: "done", ...out } : i
          )
        );
      } catch (e) {
        setItems((prev) =>
  ...
  // Descarga un resultado individual
  const download = (item) => {
  ...
  // Descarga todos los resultados listos (uno a uno)
  const downloadAll = () => {
...
const styles = `
  .cp-root { --bg:#15171c; --panel:#1d2027; --line:#2a2e38; --txt:#e6e8ec;
  ...
  .cp-head { display:flex; align-items:center; gap:8px; color:var(--accent);
    margin-bottom:4px; }
  .cp-head h2 { margin:0; font-size:18px; color:var(--txt); font-weight:600;
    letter-spacing:-.01em; }
  ...
  .cp-preset { flex:1; min-width:150px; display:flex; flex-direction:column;
    align-items:flex-start; gap:4px; padding:14px; border-radius:10px;
    background:var(--panel); border:1px solid var(--line); color:var(--txt);
    cursor:pointer; text-align:left; }
  .cp-preset:hover { border-color:var(--muted); }
  .cp-preset-sel { border-color:var(--accent); box-shadow:inset 0 0 0 1px var(--accent);
    background:var(--sel); }
  ...
  .cp-drop { display:flex; flex-direction:column; align-items:center; gap:6px;
    padding:36px; border:2px dashed var(--line); border-radius:12px;
    background:var(--panel); color:var(--muted); cursor:pointer; text-align:center;
  ...
  .cp-toolbar { display:flex; align-items:center; gap:10px; margin:18px 0 12px;
    flex-wrap:wrap; }
  .cp-btn { display:inline-flex; align-items:center; gap:7px; background:var(--accent);
    color:#0c0d10; border:0; padding:9px 15px; border-radius:8px; font-weight:600;
    font-size:13px; cursor:pointer; }
  .cp-btn:hover:not(:disabled) { filter:brightness(1.08); }
  .cp-btn:disabled { opacity:.45; cursor:not-allowed; }
  .cp-btn-ghost { background:transparent; color:var(--txt);
    box-shadow:inset 0 0 0 1px var(--line); }
  .cp-btn-ghost:hover:not(:disabled) { background:#23262f; filter:none; }
  .cp-btn-danger:hover:not(:disabled) { box-shadow:inset 0 0 0 1px var(--danger);
    color:var(--danger); }
  ...
  .cp-card { display:flex; align-items:center; gap:12px; padding:12px 14px;
    background:var(--panel); border:1px solid var(--line); border-radius:10px; }
  .cp-card-icon { color:var(--muted); display:flex; flex-shrink:0; }
  .cp-card-body { flex:1; min-width:0; }
  .cp-card-name { font-size:14px; font-weight:500; white-space:nowrap;
    overflow:hidden; text-overflow:ellipsis; }
  .cp-card-meta { font-size:12.5px; color:var(--muted); margin-top:2px;
  ...
  .cp-dl { background:transparent; border:0; color:var(--txt); cursor:pointer;
